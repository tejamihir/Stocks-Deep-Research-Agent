...
from scripts.create_vectors import (
    BALANCE_SHEET_CSV,
    CASHFLOW_STATEMENT_CSV,
    INCOME_STATEMENT_CSV,
    MODEL_NAME,
    TICKER_INFO_CSV,
    build_chroma_collections,
)
...
def _ensure_pipeline_ready() -> None:
    global _collections, _embed_model
    if _collections is not None and _embed_model is not None:
        return

    client = Client()
    model = SentenceTransformer(MODEL_NAME)
    _collections = build_chroma_collections(
        client,
        model,
        balance_sheet_csv=BALANCE_SHEET_CSV,
        income_statement_csv=INCOME_STATEMENT_CSV,
        cashflow_csv=CASHFLOW_STATEMENT_CSV,
        ticker_info_csv=TICKER_INFO_CSV,
    )
    _embed_model = model
...
def get_collections() -> Dict[str, object]:
    ...
    return _collections  # type: ignore[return-value]
...
def get_balance_sheet_metrics(ticker: str, metrics: set[str]) -> dict[str, Optional[str]]:
    collection = get_collections().get("balance_sheet")
...
def _fetch_metric_from_collection(ticker: str, collection_name: str, metric_name: str) -> Optional[str]:
    ...
    collection = get_collections().get(collection_name)
    if collection is None:
        return None
...
def retrieve_across_collections(query, top_k=3, where={}):
    ...
    query_emb = get_embed_model().encode([query]).tolist()
    ...
    for name, col in get_collections().items():
...
def get_ticker_classification(ticker: str) -> Optional[dict]:
    ticker_info_collection = get_collections().get("ticker_info")
